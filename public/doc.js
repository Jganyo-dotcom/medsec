...
async function checkServer() {
  try {
    const res = await fetch(`${API_BASE}/module1/getAll-patient`, {
      method: "GET",
...
async function fetchPatients(page = 1, q = "") {
  const url = new URL(`${API_BASE}/module1/getAll-patient`);
  ...
  if (!res.ok) {
    ...
    alert(errData.error || errData.message || "Error loading patients");
  ...
  elements.patientTbody.innerHTML = items
    .map((p) => {
      ...
      <tr>
        ...
        <td>${escapeHtml(p.gender || "")} ${age ? "/ " + age : ""}</td>
...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    const payload = {
      ...
      addresse: form.address.value.trim(),
      occupation: form.occupation.value.trim(),
      tribe: form.tribe.value.trim(),
      informant: form.informant.value.trim(),
      wardAssignment: form.clinic.value,
    };

    try {
      const token = localStorage.getItem("authToken");

      const res = await fetch(`${API_BASE}/module1/create-patients`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          ...(token ? { Authorization: `Bearer ${token}` } : {}),
